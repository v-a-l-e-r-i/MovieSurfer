...
class GenreScreenModel(BaseScreenModel):
    """
    Implements the logic of the
    :class:`~View.genre_screen.GenreScreen.GenreScreenView` class.
    """
    data = []
    genre_ = ['Біографія', 'Бойовик', 'Вестерн', 'Воєнний', 'Детектив',
             'Документальний', 'Драма', 'Історія', 'Комедія', 'Кримінал',
             'Мелодрама', 'Містика', 'Мюзикл', 'Пригоди', 'Сімейний',
             'Спорт', 'Трилер', 'Жахи', 'Фантастика', 'Фентезі']
    ...
    def search(self, genre):
        ...
        try:
            ...
            connection.autocommit = True
            cursor = connection.cursor()
            cursor.execute(f"""SELECT title, photo, grade
                                       FROM films 
